#==========================================================
# irt_ace.R
# Subroutine for master function IRT_twin()
#
# Analysis of item-level twin data under the ACE model
...
# calling the master function IRT_twin)
#
# BayesTwin package
#==========================================================
...
irt_ace <- function(data_mz, data_dz, n_burnin, n_iter, ge, irt_model,
    ...
    jags_model_irt_ace <- paste("model{
        ...
        ",ifelse(ge,"
            ...
            for (twin in 1:2){
                pheno_mz[fam,twin] ~ dnorm(f_mz[fam],tau_e_mz[fam])   
            }
        ","
            for (twin in 1:2){
    ...
    jags <- jags.model(jags_file_irt_ace, jags_data, inits = inits, n.chains = n_chains, quiet=FALSE)
