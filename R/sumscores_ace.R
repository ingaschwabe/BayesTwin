...
sumscores <- function(data_mz, data_dz, n_burnin, n_iter, ge){
    ...
    #==========================================================
    jags_model_sumscores <- paste("model{
        ##MZ twins
        for (fam in 1:n_mz){
    ...
    if(ge == FALSE){
        ...
        output = list(results = results, samples_var_a = samples_var_a, samples_var_c = samples_var_c, 
                      ...
                      hpd_var_a = hpd_var_a, hpd_var_c = hpd_var_c, hpd_var_e = hpd_var_e, hpd_mu = hpd_mu)
        ...
        #Put results in a table 
        results = matrix(c(var_a, sd_var_a, var_c, sd_var_c, beta0, sd_beta0, beta1, sd_beta1, mu, sd_mu), 2, 5)
        colnames(results) <- c("varA","varC","beta0", "beta1", "Mu")
        rownames(results) <- c("Posterior mean","Posterior standard deviation")
        results = as.table(results) 
