...
#
# Analysis of twin data under the ADE model using 
...
irt_ade_cov <- function(data_mz, data_dz, 
    ...
    INV_GAMMA = FALSE; UNIF_noGE = FALSE; INV_GAMMA_noGE = FALSE
    ...
    #==========================================================
    jags_model_irt_ade_cov <- paste("model{
        ##MZ twins
        for (fam in 1:n_mz){
        ...
        ",ifelse(ge,"
            ...
            pheno_dz[fam,1] ~ dnorm(g2[fam,1] + inprod(X_dz_twin1[fam,], b[1:N_cov]), tau_e_dz[fam,1]) 
            pheno_dz[fam,2] ~ dnorm(g2[fam,2] + inprod(X_dz_twin2[fam,], b[1:N_cov]), tau_e_dz[fam,2])      
        ","
            pheno_dz[fam,1] ~ dnorm(g2[fam,1] + inprod(X_dz_twin1[fam,], b[1:N_cov]), tau_e) 
            pheno_dz[fam,2] ~ dnorm(g2[fam,2] + inprod(X_dz_twin2[fam,], b[1:N_cov]), tau_e)
        "),"                    
        ",ifelse(PL_1,"
        #1pl model twin1 (DZ)
        for (j in 1:n_items){
    ...
    jags <- jags.model(jags_file_irt_ade_cov, jags_data, inits = inits, n.chains = n_chains, quiet=FALSE)
    ...
    if(ge == FALSE){
        ...
        if(n_chains > 1){
            ...
            samples_var_d = 1/c(out$tau_d[,,1:n_chains])
        ...
        sd_var_d = sd(samples_var_d)
        sd_var_e = sd(samples_var_e)
        
        #Calculate HPD: 
        hpd_var_a = HPD(samples_var_a, 0.95)
        hpd_var_d = HPD(samples_var_d, 0.95)
        ...
        results = matrix(c(var_a, sd_var_a, hpd_var_a[1],hpd_var_a[2], 
                           var_d, sd_var_d, hpd_var_d[1],hpd_var_d[2],
        ...
        if (fit_stats == FALSE){
            if(PL_2 == TRUE || GPCM == TRUE){
                output = list(results = results, results_b = results_b,
                              ...
                              samples_var_a = samples_var_a, samples_var_d = samples_var_d, 
                              samples_var_e = samples_var_e, samples_b = samples_b,
                              samples_item_b = samples_item_b, samples_alpha = samples_alpha,
                              
                              var_a = var_a, var_d = var_d, var_e = var_e, 
                              item_b = item_b, alpha = alpha, b = b,
                              
                              sd_var_a = sd_var_a, sd_var_d = sd_var_d, sd_var_e = sd_var_e, 
                              sd_item_b = sd_item_b, sd_alpha = sd_alpha, sd_b = sd_b,
                              
                              hpd_var_a = hpd_var_a, hpd_var_d = hpd_var_d, 
        ...
        if(fit_stats == FALSE){
            output = list(results = results, results_b = results_b,
                          ...
                          var_a = var_a, var_d = var_d, beta0 = beta0, beta1 = beta1, 
                          item_b = item_b, b = b,
                          
                          sd_var_a = sd_var_a, sd_var_d = sd_var_d, 
