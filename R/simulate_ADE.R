...
simulate_ADE = function(n_mz, n_dz, var_a, var_d,  var_e,
    ...
    #Generate answers for each twin, ADE model
    ...
    if(n_items > 0 && n_var > 0){
        ...
        for (i in 1:n_mz){
            pheno_mz[i, 1] = rnorm(1, g_mz[i] + (cov_mz[i,1:n_var] %*% beta), sqrt(var_e_mz))
            pheno_mz[i, 2] = rnorm(1, g_mz[i] + (cov_mz[i,(n_var+1):(2*n_var)] %*% beta), sqrt(var_e_mz))
        }
        ...
        for (i in 1:n_dz){
            pheno_dz[i,1] = rnorm(1, g2[i,1] + (cov_dz[i,1:n_var] %*% beta), sqrt(var_e_dz_twin1))
            pheno_dz[i,2] = rnorm(1, g2[i,2] + (cov_dz[i,(n_var+1):(2*n_var)] %*% beta) , sqrt(var_e_dz_twin2))
        }
        ...
        ### II. DZ twins: 
        pheno_dz = cbind(rnorm(n_dz, g2[,1], sqrt(var_e_dz_twin1)),
                         rnorm(n_dz, g2[,2], sqrt(var_e_dz_twin2)))
        ...
        for (i in 1:n_mz){
            y_mz[i, 1] = rnorm(1, g_mz[i] + (cov_mz[i,1:n_var] %*% beta), sqrt(var_e_mz))
            y_mz[i, 2] = rnorm(1, g_mz[i] + (cov_mz[i,(n_var+1):(2*n_var)] %*% beta), sqrt(var_e_mz))
        }
        ...
        for (i in 1:n_dz){
            y_dz[i,1] = rnorm(1, g2[i,1] + (cov_dz[i,1:n_var] %*% beta), sqrt(var_e_dz_twin1))
            y_dz[i,2] = rnorm(1, g2[i,2] + (cov_dz[i,(n_var+1):(2*n_var)] %*% beta) , sqrt(var_e_dz_twin2))
        }
    
    }   else { 
        ### I. MZ twins: 
        y_mz = cbind(rnorm(n_mz, g_mz,  sqrt(var_e_mz)), 
                     rnorm(n_mz, g_mz, sqrt(var_e_mz)))
